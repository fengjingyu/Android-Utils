...
import com.jingyu.android.init.middle.config.okhttp.MyCallback;
import com.jingyu.android.init.middle.config.okhttp.loading.DialogManager;
import com.jingyu.android.init.middle.config.okhttp.loading.HttpLoadingDialog;
...
public abstract class BaseRespHandler<T> extends MyRespHandler<T> {
    ...
    public boolean onMatchAppStatusCode(MyReqInfo myReqInfo, MyRespInfo myRespInfo, T resultBean) {
        ...
        if (resultBean instanceof IRespMsgCode) {
            if (UtilString.equals(((IRespMsgCode) resultBean).getCode(), REQ_SUCCESS)) {
                return true;
            } else {
