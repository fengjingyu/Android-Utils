package com.jingyu.utilstest.search;
...
public class SearchRecordDb extends SQLiteOpenHelper {
    ...
    public SearchRecordDb(Context context, String operatorTableName) {
    ...
    public ContentValues createContentValue(SearchRecordBean model) {
    ...
    public SearchRecordBean createModel(Cursor c) {
        SearchRecordBean model = new SearchRecordBean();
     ...
     * 插入一条记录
     */
    public long insert(SearchRecordBean model) {
     ...
     * 查询所有
     */
    public List<SearchRecordBean> queryAllByDESC() {
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.query(mOperatorTableName, null, null, null, null, null, _ID + SORT_DESC); // 条件为null可以查询所有
        List<SearchRecordBean> beans = new ArrayList<SearchRecordBean>();
        while (c.moveToNext()) {
            SearchRecordBean bean = createModel(c);
            beans.add(bean);
        }
        c.close();
        db.close();
     ...
     * 查询所有
     */
    public List<SearchRecordBean> queryAllByASC() {
        SQLiteDatabase db = getReadableDatabase();
     ...
     * 根据关键字查询
     */
    public List<SearchRecordBean> query(String value) {
        SQLiteDatabase db = getReadableDatabase();
     ...
     * 分页查找
     */
    public List<SearchRecordBean> queryPage(int pageNum, int capacity) {
    ...
    public int update(SearchRecordBean model, String value) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = createContentValue(model);
