package com.jingyu.android.init.middle.config.okhttp.loading;
...
public class DialogManager {
    ...
    public void mayShow(String uniqueTag) {
        ...
        if (isAllRequestEnd()) {
            ...
            mHttpDialog.show();
            Logger.d(MyCallback.TAG_HTTP, this.toString() + "---showHttpDialog()");
        } else {
    ...
    private void closeHttpDialog() {
        if (mHttpDialog != null && mHttpDialog.isShowing()) {
            mHttpDialog.cancel();
            Logger.d(MyCallback.TAG_HTTP, this.toString() + "---closeHttpDialog()");
        }
    }
